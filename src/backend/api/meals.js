...
router.get("/", (request, response) => {
	...
	// Returns all meals
		...
		pool.query(
			`SELECT  meal.id, meal.title, meal.description, meal.location, meal.price, review.stars FROM meal
...
router.post("/", (request, response) => {
	...
	pool.query("INSERT INTO meal SET ?", meal, function(error, results, fields) {
		if (error) {
			console.log("error", error);
		} else return response.json(results);
		// console.log(results);
